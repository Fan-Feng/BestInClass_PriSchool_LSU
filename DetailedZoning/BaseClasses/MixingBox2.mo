  ...
  Buildings.Fluid.Actuators.Dampers.Exponential damRec(
    ...
    annotation (
      Placement(transformation(
        ...
        extent={{-10,-10},{10,10}},
        rotation=90)));
  Modelica.Blocks.Interfaces.RealInput yRet annotation (Placement(
        transformation(
        extent={{-20,-20},{20,20}},
        rotation=-90,
        origin={-60,120})));
  Modelica.Blocks.Interfaces.RealInput yOut annotation (Placement(
        transformation(
        extent={{-20,-20},{20,20}},
        rotation=-90,
        origin={0,120})));
  Modelica.Blocks.Interfaces.RealInput yExh annotation (Placement(
        transformation(
        extent={{-20,-20},{20,20}},
        rotation=-90,
        origin={50,120})));
...
equation
  ...
  connect(damRec.port_b, port_Sup) annotation (Line(
      points={{62,10},{62,60},{100,60}},
      color={0,127,255}));
  connect(port_Ret, damExh.port_a) annotation (Line(
      points={{100,-60},{-20,-60}},
      color={0,127,255}));
  connect(port_Ret, damRec.port_a) annotation (Line(
      points={{100,-60},{62,-60},{62,-10}},
      color={0,127,255}));

  connect(yRet, damRec.y) annotation (Line(points={{-60,120},{-60,44},{50,44},{
          50,7.77156e-16}}, color={0,0,127}));
  connect(yOut, damOA.y)
    annotation (Line(points={{0,120},{0,72}}, color={0,0,127}));
  connect(yExh, damExh.y) annotation (Line(points={{50,120},{50,46},{-30,46},{
          -30,-48}}, color={0,0,127}));
