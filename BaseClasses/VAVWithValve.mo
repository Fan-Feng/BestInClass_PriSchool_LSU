...
model VAVWithValve
  ...
  Buildings.Fluid.HeatExchangers.DryCoilEffectivenessNTU terHea(
    ...
    m2_flow_nominal=m_flow_nominal*1006*(28 - 12)/4200/(55 - 45),
    Q_flow_nominal=m_flow_nominal*1006*(28 - 12),
    ...
    T_a1_nominal=328.15,
    T_a2_nominal=285.15) "Heat exchanger of terminal box" annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-44,-2})));
  ...
  Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage val(
    ...
    dpValve_nominal(displayUnit="Pa") = 200,
    m_flow_nominal=m_flow_nominal*1006*20/4200/10,
    dpFixed_nominal=0,
    from_dp=true,
    use_inputFilter=false) "VaV valve" annotation (Placement(transformation(
        extent={{8,-7},{-8,7}},
        rotation=90,
        origin={27,22})));
...
equation
  ...
  connect(port_a1, val.port_a)
    annotation (Line(points={{100,40},{27,40},{27,30}}, color={0,127,255}));
  connect(u, val.y) annotation (Line(points={{-120,-40},{-70,-40},{-70,22},{
          18.6,22}}, color={0,0,127}));
  connect(val.port_b, massFlowRate.port_a)
    annotation (Line(points={{27,14},{27,8},{0,8}}, color={0,127,255}));
  connect(massFlowRate.port_b, terHea.port_a2)
    annotation (Line(points={{-20,8},{-38,8}}, color={0,127,255}));
  connect(terHea.port_b2, port_b1) annotation (Line(points={{-38,-12},{-6,-12},
          {-6,-44},{100,-44},{100,-40}}, color={0,127,255}));
