...
model AHU_Gas_furnace
  ...
  Modelica.Blocks.Interfaces.RealInput TSet "Gas_furnace Control" annotation (
      Placement(transformation(extent={{-220,-304},{-180,-264}}),
        iconTransformation(extent={{-120,-100},{-100,-80}})));
  Buildings.Fluid.Sensors.VolumeFlowRate senSupheat(redeclare package Medium =
        MediumA, m_flow_nominal=m_flow_nominal)
    "Sensor for supply fan flow rate"
    annotation (Placement(transformation(extent={{66,52},{86,72}})));
  Buildings.Fluid.Boilers.BoilerPolynomial gas_furnace(
    a={0.9},
    ...
    from_dp=true,
    T_start=293.15)
    annotation (Placement(transformation(extent={{120,52},{140,72}})));
  Buildings.HeatTransfer.Sources.FixedTemperature TAmb(T=288.15)
    "Ambient temperature in boiler room"
    annotation (Placement(transformation(extent={{48,142},{68,162}})));
  Modelica.Blocks.Sources.RealExpression PBoi(y=senSupheat.V_flow*4200*(
        gas_furnace.T - TMix.T))                  "Boiler gas consumption"
    annotation (Placement(transformation(extent={{128,0},{148,20}})));
  Modelica.Blocks.Interfaces.RealOutput gas_consumption
    annotation (Placement(transformation(extent={{480,-28},{500,-8}}),
...
equation
  ...
  connect(senSupheat.port_a, TMix.port_b)
    annotation (Line(points={{66,62},{56,62}}, color={0,127,255}));
  connect(swiFreSta.y, gas_furnace.y) annotation (Line(points={{82,-82},{108,-82},
          {108,70},{118,70}}, color={0,0,127}));
  connect(gas_furnace.port_b, cooCoi.port_a2) annotation (Line(points={{140,62},
          {140,60},{184,60},{184,72},{196,72},{196,62}}, color={0,127,255}));
  connect(gas_furnace.port_a, senSupheat.port_b)
    annotation (Line(points={{120,62},{86,62}}, color={0,127,255}));
  connect(TAmb.port, gas_furnace.heatPort)
    annotation (Line(points={{68,152},{130,152},{130,69.2}}, color={191,0,0}));
  connect(PBoi.y, gas_consumption) annotation (Line(points={{149,10},{149,8},{196,
          8},{196,36},{468,36},{468,-18},{490,-18}}, color={0,0,127}));
