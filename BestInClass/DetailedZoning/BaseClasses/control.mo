...
<a href=\"modelica://Modelica.Blocks.Examples.RealNetwork1\">Modelica.Blocks.Examples.RealNetwork1</a>.
    ...
    equation
      ...
      connect(Off2One.outPort, iOn[1].inPort[1]) annotation (Line(points={{-50,
              42.5},{-50,42.5},{-50,26},{0.25,26},{0.25,13}},
                                                        color={0,0,0}));
      connect(iOn[1].outPort[2], One2Off.inPort) annotation (Line(points={{-0.125,
              -8.5},{2,-8.5},{2,-20},{52,-20},{52,44}},
                                                   color={0,0,0}));
      connect(nmin12n.inPort, iOn[n-1].outPort[1]) annotation (Line(points={{-50,-50},
              {-50,-40},{0.125,-40},{0.125,-8.5}},  color={0,0,0}));
      ...
      connect(N2Nmin1.outPort, iOn[n-1].inPort[2]) annotation (Line(points={{40,
              -58.5},{40,-58.5},{40,16},{-0.25,16},{-0.25,13}},
                                                     color={0,0,0}));
...
First implementation.
  ...
  model SimpleSingleBoilerControl "Simple single boiler control"
    parameter Real enableThreshold = 1 "Enable above this load [W]";
    parameter Real disableThreshold = 0.5
                                         "Disable below this load [W]";

    Modelica.Blocks.Interfaces.RealOutput y[1](each start=0)
      annotation (Placement(transformation(extent={{100,-10},{120,10}})));
    Modelica.Blocks.Interfaces.BooleanInput On
      annotation (Placement(transformation(extent={{-140,60},{-100,100}})));
    Modelica.Blocks.Interfaces.RealInput loa
      annotation (Placement(transformation(extent={{-140,-20},{-100,20}})));
    Modelica.Blocks.Interfaces.RealInput sta[1] "Current boiler status"
      annotation (Placement(transformation(extent={{-140,-100},{-100,-60}})));

    Modelica.Blocks.Logical.Hysteresis loadHysteresis(
      uLow = disableThreshold,
      uHigh = enableThreshold)
      annotation (Placement(transformation(extent={{-40,-14},{-20,6}})));

    Modelica.Blocks.Logical.And boilerLogic
  ...
  equation
    connect(loa, loadHysteresis.u);
    connect(loadHysteresis.y, boilerLogic.u1);
    connect(On, boilerLogic.u2);
    connect(boilerLogic.y, boolToReal.u);
    connect(boolToReal.y, y[1]);

  end SimpleSingleBoilerControl;

  model SecPumCon
    "This model is used for secondary chilled water pump control."
    parameter Real tWai = 300 "Waiting time";

      parameter Integer n=3
      "the number of pumps";
    MultizoneOfficeComplexAir.BaseClasses.HVACSide.BaseClasses.Component.WaterSide.Control.PumpStageN
      pumSta(
      tWai=tWai,
      thehol_up=0.9,
      n=n,
  ...
  equation
    ...
    connect(conPI.set, dpSet) annotation (Line(points={{16,-10},{-20,-10},
            {-20,-40},{-120,-40}}, color={0,0,127}));
    connect(conPI.y, y[1]) annotation (Line(points={{39,-10},{60,-10},{60,0},{
            110,0},{110,-3.33333}}, color={0,0,127}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics={
            Rectangle(
            extent={{-100,100},{100,-100}},
            lineColor={0,0,127},
  ...
  model PlantStageN_Copy
    "Combined stage controller for N chillers or N boilers"
    parameter Real tWai "Waiting time";
    ...
    Modelica.Blocks.Interfaces.RealInput loa
      annotation (Placement(transformation(extent={{-140,-20},{-100,20}})));
    BestInClass.DetailedZoning.BaseClasses.Control.PlantStageCondition
      plantNStageCondition(
      thehol=thehol,
      n=n,
      Cap=Cap) annotation (Placement(transformation(extent={{-60,-16},{-40,4}})));
    BestInClass.DetailedZoning.BaseClasses.Control.BaseClasses.StageN
      nSta(tWai=tWai, n=n)
  ...
  equation
    ...
    connect(plantNStageCondition.Status, sta) annotation (Line(points={{-62,
            -12},{-80,-12},{-80,-80},{-120,-80}}, color={0,0,127}));
    connect(plantNStageCondition.Off, nSta.Off) annotation (Line(points={{-39,-10},
            {-24,-10},{-24,-12},{-8,-12}},            color={255,0,255}));
    connect(plantNStageCondition.On, nSta.On)
      annotation (Line(points={{-39,-2},{-24,-2},{-24,0},{-8,0}},
                                                   color={255,0,255}));
    annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
              -100},{100,100}})),           Icon(coordinateSystem(
            preserveAspectRatio=false, extent={{-100,-100},{100,100}}), graphics={
